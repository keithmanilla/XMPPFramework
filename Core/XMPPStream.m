...
- (instancetype)initWithStream:(XMPPStream *)xmppStream uniqueID:(NSString *)uniqueID myJID:(XMPPJID *)myJID;
...
- (void)continueSendIQ:(XMPPIQ *)iq inContextOfEventWithID:(NSString *)eventID withTag:(long)tag
{
    ...
    XMPPElementEvent *event = [[XMPPElementEvent alloc] initWithStream:self uniqueID:eventID myJID:self.myJID];
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	{
        XMPPElementEvent *event = [[XMPPElementEvent alloc] initWithStream:self uniqueID:[self generateUUID] myJID:self.myJID];
